#!/usr/bin/env bash
# Repeatable GNOME release scenario for Browser Picker.
# Run from an actual GNOME graphical session after `nix build .#browser-picker`.
...
case ":${desktop}:" in
  *:[Gg][Nn][Oo][Mm][Ee]:*) ;;
  *)
    echo "This scenario requires an actual GNOME session, not ${desktop:-an unknown desktop}." >&2
    exit 1
    ;;
esac
case ":${desktop}:" in
  *:[Kk][Dd][Ee]:*|*:[Pp][Ll][Aa][Ss][Mm][Aa]:*|*:[Nn][Ii][Rr][Ii]:*)
    echo "Refuse KDE, Plasma, niri, and mixed-session substitutes: XDG_CURRENT_DESKTOP=$desktop" >&2
    exit 1
    ;;
esac
if [ "$session_type" = "tty" ]; then
  echo "Refuse a tty session; GNOME must own the graphical session." >&2
  exit 1
fi
if ! pgrep -u "$(id -u)" -x gnome-shell >/dev/null 2>&1; then
  echo "gnome-shell is not running for this user; start a GNOME session." >&2
  exit 1
fi
if [ -z "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]; then
  echo "GNOME must provide DISPLAY or WAYLAND_DISPLAY." >&2
  exit 1
fi
if [ -z "${DBUS_SESSION_BUS_ADDRESS:-}" ]; then
  echo "GNOME must provide a user session D-Bus." >&2
  exit 1
fi
...
snapshot_dir="$(mktemp -d "${TMPDIR:-/tmp}/browser-picker-gnome-snapshot.XXXXXX")"
snapshot_operator() {
  local path="$1"
  local name="$2"
  if [ -e "$path" ]; then
...
snapshot_operator "$operator_config/gnome-mimeapps.list" gnome-mimeapps.list
snapshot_operator "$operator_home/.config/mimeapps.list" home-mimeapps.list

workdir="$(mktemp -d "${TMPDIR:-/tmp}/browser-picker-gnome.XXXXXX")"
cleanup() {
  ...
  unchanged=0
  for name in mimeapps.list gnome-mimeapps.list home-mimeapps.list; do
    case "$name" in
    ...
    if [ -e "$snapshot_dir/$name" ]; then
      if ! cmp -s "$snapshot_dir/$name" "$current"; then
        echo "operator $current changed during the GNOME scenario" >&2
        unchanged=1
      fi
    elif [ -e "$current" ]; then
      echo "operator $current was created during the GNOME scenario" >&2
      unchanged=1
    fi
  done
...
tree="$(inspect dump)"
printf '%s\n' "$tree" | grep -F "GNOME Settings"
printf '%s\n' "$tree" | grep -F "Apps"
printf '%s\n' "$tree" | grep -F "Default Apps"
printf '%s\n' "$tree" | grep -F "never changes system defaults"
printf '%s\n' "$tree" | grep -Fi "set as default" && {
  echo "GNOME instructions offered a takeover action" >&2
  exit 1
} || true
...
"$PICKER" "https://gnome-first.example/" &
picker_pid=$!
...
grep -F "https://gnome-first.example/" "$BROWSER_PICKER_TEST_OUTPUT"
...
echo "GNOME session scenario completed without changing operator defaults"
